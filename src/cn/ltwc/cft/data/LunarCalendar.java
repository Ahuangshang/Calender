...
public class LunarCalendar {
	...
	final static String chineseLNumber[] = { "正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊" };
	...
	public String getLunarDate(int year_log, int month_log, int day_log, boolean isday) {
		...
		if (!isday) {
			...
			if (month_log < 13 && month_log > 0) {
				String jieqi = new JieQi(year_log, month_log, day_log).getSolartermsMsg();
	 ...
	 */
	@SuppressWarnings("unused")
	public String getCalendarInfoByChooseDay(int chooseYear, int chooseMonth, int chooseDay) {
		String lunarDate = getLunarDate(chooseYear, chooseMonth, chooseDay, true);// 得到当前的农历的日期(先执行此方法，通过传入阳历数据计算得到农历信息)
		...
		int temp = SpecialCalendar.getInstance().getWeekdayOfMonth(chooseYear, chooseMonth, chooseDay);// 得到该天星期几
		String week = "";
		switch (temp) {
		case 0:
			week = "星期日";
	 ...
	 * @return 农历信息(包含农历年份月份日期以及周几)
	 */
	public RiqiBean getRiqiBeanInfo(int chooseYear, int chooseMonth, int chooseDay) {
		String lunarDate = getLunarDate(chooseYear, chooseMonth, chooseDay, true);// 得到当前的农历的日期(先执行此方法，通过传入阳历数据计算得到农历信息)
		String lunarMonth = getLunarMonth();// 得到当前的农历的月份
		if (lunarMonth.equals(lunarDate)) {
			// 如果当前的农历日期是月份，则返回初一
		...
		bean.setnHolidayDay(getLunarDate(chooseYear, chooseMonth, chooseDay, false));
