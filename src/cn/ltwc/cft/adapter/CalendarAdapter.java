...
public class CalendarAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position < daysOfMonth + dayOfWeek && position >= dayOfWeek) {
			...
			for (int i = 0; i < Constant.lunarFajia.length; i++) {
				for (int j = 0; j < dayNumber.length; j++) {
					String dx = dayNumber[j].split("\\.")[1];
					String lh = Constant.lunarFajia[i];
					if (lh.contains(dx)) {
						list.add(j);
						if (lh.contains("春节")) {// 如果包含春节，则后面的两天跟着放假
							list.add(j + 1);
							list.add(j + 2);
						}
						if (lh.contains("皇上生辰")) {// 如果包含皇上生辰，则后面的七天跟着放假
							list.add(j + 1);
							list.add(j + 2);
							list.add(j + 3);
							list.add(j + 4);
			...
			// 中国阳历放假的日子
			for (int i = 0; i < Constant.solarFajia.length; i++) {
				for (int j = 0; j < dayNumber.length; j++) {
					String dx = dayNumber[j].split("\\.")[1];
					String lh = Constant.solarFajia[i];
					if (lh.contains(dx)) {
						list.add(j);
						if (lh.contains("国庆")) {// 如果是国庆，则后面两天也放假
							list.add(j + 1);
							list.add(j + 2);
						}
					}
